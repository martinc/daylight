...
struct LocationSample {
    ...
    let date: Day
    let sunrise: Time, sunset: Time, noon: Time, dawn: Time, dusk: Time
    let eot: Double
...
class DaylightTests: XCTestCase {
    ...
    func testDateInterface() {
        for sample in sampleData {
            ...
            let noon = sample.dateAtMidnight.timeOf(.noon, at: sample.location)
            ...
            checkTimesMatch(date: noon, time: sample.noon, timezone: timezone)
    ...
    func testDayInterface() {
        for sample in sampleData {
            ...
            let noon = sample.date.timeOf(.noon, at: sample.location)
    ...
    func testFormatting() {
        ...
        let outputString = dateFormatter.string(from: sunrise)
        let targetString = "July 1, 2015 at 5:28:39 AM EDT"
        XCTAssertEqual(outputString, targetString)
    }
